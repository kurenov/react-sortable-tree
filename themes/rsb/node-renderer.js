...
class NodeRendererRSB extends Component {
  ...
  render() {
    ...
    //     handle = connectDragSource(<span className="material-icons" style={{ marginLeft: 16, paddingTop: 7 }}>{node?.children?.length ? 'list_alt' : 'local_offer'}</span>, {
    //       dropEffect: 'copy',
    //     });
    //   }
    // }
    ...
    const handle = (
      <span
        ...
        style={{ marginLeft: 16, paddingTop: 7 }}
      >
        {node?.children?.length ? 'list_alt' : 'local_offer'}
      </span>
    );
